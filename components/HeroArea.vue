      ...
      <div class="grid lg:grid-cols-2 gap-16 items-center">
        <div>
          ...
          <div class="flex flex-wrap gap-6 mb-10 reveal reveal-delay-3">
            <div v-for="stat in stats" :key="stat.labelKey" class="text-center">
              ...
              <div class="text-xs text-[var(--color-text-muted)] uppercase tracking-wider mt-1">{{ $t(stat.labelKey) }}</div>
            </div>
          </div>

          <!-- CTAs -->
          <div class="flex flex-wrap gap-4 reveal reveal-delay-4">
            <a href="#works" class="btn-primary" @click.prevent="onViewWorks">
            ...
            <NuxtLink :to="$t('doc.pdf')" class="btn-outline" @click="analytics.fileDownload($t('doc.pdf'), 'hero_short_cv')">
              <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
        ...
        <div class="hidden lg:flex justify-center reveal reveal-delay-2">
          <div class="relative">
            ...
            <img src="/assets/img/hero/heroRight.png" alt="Hero" class="relative max-w-lg w-full h-auto drop-shadow-2xl" />
          </div>
        </div>
      </div>
    </div>
...
function onViewWorks() {
  analytics.ctaClick('view_works', 'hero')
  smoothScroll('#works')
}
